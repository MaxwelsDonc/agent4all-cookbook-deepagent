...
```text
帮我调研 Deep Agents 官方资料，整理一份小白教程大纲，
再把每一节课的示例代码和常见错误也列出来。
```
...
```mermaid
flowchart TD
  U[用户给出复杂任务] --> P[Planning 拆成待办]
  P --> T{需要外部能力吗}
  T -->|需要| Tool[Tools 调用工具]
  Tool --> FS[Filesystem 保存结果]
  T -->|不需要| W[继续思考]
  FS --> C[Context management 管理上下文]
  W --> C
  C --> S{需要分工吗}
  S -->|需要| Sub[Subagent 处理子任务]
  Sub --> C
  S -->|不需要| A[生成下一步]
  A --> H{高风险操作吗}
  H -->|是| HITL[Human-in-the-loop 先问人]
  H -->|否| Done[完成任务]
  HITL --> Done
```
...
下一节是本章自测。通过自测后，我们再进入第二章，开始准备环境并跑通第一个最小 Agent。
